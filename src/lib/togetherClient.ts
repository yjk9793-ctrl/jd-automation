...
export class TogetherAILLMClient {
  ...
  constructor() {
    ...
    console.log('=== TogetherAILLMClient Constructor ===');
    console.log('TOGETHER_API_KEY exists:', !!this.apiKey);
    console.log('TOGETHER_API_KEY length:', this.apiKey.length);
    
    if (!this.apiKey || this.apiKey === '') {
      console.error('❌ TOGETHER_API_KEY is not set!');
      const error = new Error('TOGETHER_API_KEY is not set in environment variables');
      error.name = 'TogetherAPIKeyError';
      throw error;
    }
    
    console.log('✅ TOGETHER_API_KEY is set, initializing...');
    console.log('✅ Together AI client initialized');
  }

  async analyzeJobDescription(content: string, type: AnalysisType): Promise<any> {
    try {
      console.log('=== Together AI Analysis Started ===');
      ...
      console.log('✅ Together AI analysis completed');
      return parsedData;
    } catch (error) {
      console.error('Together AI API error:', error);
      throw new Error('Failed to analyze with Together AI API');
    }
  }
